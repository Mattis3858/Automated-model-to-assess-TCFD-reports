...
def compute_accuracy_new(xlsx_path, pred_xlsx_path):
    """
    使用新的分組邏輯計算準確率。

    Args:
        xlsx_path (str): 真實答案的 Excel 檔案路徑。
        pred_xlsx_path (str): 預測結果的 Excel 檔案路徑。
    """
    try:
        truth_df = pd.read_excel(xlsx_path)
        # 將讀取 CSV 的程式碼替換為讀取 Excel
    ...
    for col in key_cols + [pred_col]:
        if col not in pred_df.columns:
            # 更新錯誤訊息中的變數名稱
            print(f"錯誤: 預測檔案 '{pred_xlsx_path}' 中缺少欄位 '{col}'。")
            return

    # 排序以確保資料對齊
    truth_df = truth_df.sort_values(by=key_cols).reset_index(drop=True)
    pred_df = pred_df.sort_values(by=key_cols).reset_index(drop=True)
    ...
    for label, group in merged_df.groupby('Label'):
        # 修正註解與變數使其一致：只取每個 Label 的前 10 個 chunks
        top10_chunks = group.head(10)

        # 提取真實值和預測值，並將 NaN (可能由 outer join 產生) 替換為 'N'
        true_vals = top10_chunks['y_true'].fillna('N').tolist()
        pred_vals = top10_chunks['y_pred'].fillna('N').tolist()
